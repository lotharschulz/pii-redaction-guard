...
class ReversibleAnonymizer:
    ...
    def _deduplicate_results(self, results: list[RecognizerResult]) -> list[RecognizerResult]:
        ...
        deduplicated: list[RecognizerResult] = []
            ...
            self._audit_log.append(
                {
                    ...
                    "entities_found": list({d["entity_type"] for d in detections}),
                    "details": detections,
                }
            )
            print(
                f"  [Presidio Input] Redacted {len(detections)} PII entities: "
    ...
    def process_output(self, llm_output: str) -> str:
        ...
        if final_detections:
            ...
            if self.allow_restored_pii and self.reversible and self._anonymizer:
                ...
                for detection in final_detections:
                    ...
                    if is_known:
                        ...
                        for known_start, known_end in known_ranges:
                            # Check for overlap
                            if not (
                                detection["end"] <= known_start or detection["start"] >= known_end
                            ):
                                overlaps_known = True
                                break
                ...
                if new_detections:
                    # Deduplicate overlapping new detections
                    # (e.g., email detected as both EMAIL and URL)
                    deduplicated_new: list[dict] = []
                    for detection in sorted(
                        new_detections, key=lambda d: (d["start"], -d["score"])
                    ):
                        overlaps = False
                        for accepted in deduplicated_new:
                            if not (
                                detection["end"] <= accepted["start"]
                                or detection["start"] >= accepted["end"]
                            ):
                                overlaps = True
                                break
                        if not overlaps:
                    ...
                    self._audit_log.append(
                        {
                            ...
                            "entities_found": list({d["entity_type"] for d in deduplicated_new}),
                            "details": deduplicated_new,
                        }
                    )
                    print(
                        f"  [Presidio Output] Caught {len(deduplicated_new)} "
                        f"NEW PII entities (hallucinations)"
                    )
                    ...
                    for detection in sorted(
                        deduplicated_new, key=lambda d: d["start"], reverse=True
                    ):
                ...
                self._audit_log.append(
                    {
                        ...
                        "entities_found": list({d["entity_type"] for d in final_detections}),
                        "details": final_detections,
                    }
                )
                print(
                    f"  [Presidio Output] Caught {len(final_detections)} PII entities in LLM output"
                )
                return final_clean
...
if __name__ == "__main__":
    ...
    mapping_count = len(guard_shared._anonymizer.get_mapping()) if guard_shared._anonymizer else 0
    print(f"   PROBLEM: Mapping contains {mapping_count} items from BOTH users!")
    ...
    mapping_count = len(guard_shared._anonymizer.get_mapping()) if guard_shared._anonymizer else 0
    print(f"   SECURE: Mapping contains {mapping_count} item(s) from only User 2")
